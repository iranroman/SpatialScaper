...
class AudioSynthesizer(object):
    def __init__(self, params, mixtures, mixture_setup, db_config, audio_format):
        ...
        self._outpath = (
            params["mixturepath"]
            + "/"
            + mixture_setup["scenario"]
            + "/"
            + self._audio_format
        )
        self._rirdata = db_config._rirdata
        self._nb_rooms = sum(
            os.path.isdir(self._rirpath_sofa + f"/{self._audio_format}/" + i)
            for i in os.listdir(self._rirpath_sofa + f"/{self._audio_format}/")
        )
        self._room_names = sorted(
            [
                d
                for d in os.listdir(self._rirpath_sofa + f"/{self._audio_format}/")
                if os.path.isdir(self._rirpath_sofa + f"/{self._audio_format}/" + d)
            ]
        )
        ...
        self._time_idx100 = np.arange(0.0, self._t_mix, 0.1)
    ...
    def synthesize_mixtures(self):
        ...
        for nfold in range(self._nb_folds):
            ...
            for nr in range(nb_rooms_in_fold):
                ...
                sofas = sorted(
                    os.listdir(self._rirpath_sofa + f"/{self._audio_format}/" + nroom)
                )
                channel_rirs_sofa = []
                max_long = 0
                for sofa in sofas:
                    rirs = sofa_utils.load_rir(
                        self._rirpath_sofa
                        + f"/{self._audio_format}/"
                        + nroom
                        + f"/{sofa}"
                    )
                    if len(rirs) > max_long:
                        max_long = len(rirs)
                    channel_rirs_sofa.append(
                        np.transpose(rirs, (2, 1, 0))[..., np.newaxis]
                    )
                for i in range(len(channel_rirs_sofa)):
                    diff_ = max_long - len(channel_rirs_sofa[i][0, 0])
                    channel_rirs_sofa[i] = np.pad(
                        channel_rirs_sofa[i], ((0, 0), (0, 0), (0, diff_), (0, 0))
                    )
                ...
                for nmix in range(nb_mixtures):
                    ...
                    for nev in range(nb_events):
                        ...
                        if self._db_name == "nigens":
                            ...
                            eventsig, fs_db = librosa.load(
                                filename, sr=self._fs
                            )  # here we need librosa since we are loading .mp3
                        else:
                        ...
                        riridx = (
                            mixture_nm["rirs"][nev]
                            if nb_events > 1
                            else mixture_nm["rirs"]
                        )

                        moving_condition = (
                            mixture_nm["isMoving"][nev]
                            if nb_events > 1
                            else mixture_nm["isMoving"]
                        )
                        if nb_events > 1 and not moving_condition:
                            riridx = (
                                int(riridx[0])
                                if len(riridx) == 1
                                else riridx.astype("int")
                            )
                        ...
                        if moving_condition:
                            ...
                            mixeventsig = (
                                481.6989
                                * utils.ctf_ltv_direct(
                                    eventsig,
                                    channel_rirs[:, :, riridx, ntraj],
                                    ir_times,
                                    self._fs_mix,
                                    self._stft_winsize_moving,
                                )
                                / float(len(eventsig))
                            )
                        else:
                            mixeventsig0 = scipy.signal.convolve(
                                eventsig,
                                np.squeeze(channel_rirs[:, 0, riridx, ntraj]),
                                mode="full",
                                method="fft",
                            )
                            mixeventsig1 = scipy.signal.convolve(
                                eventsig,
                                np.squeeze(channel_rirs[:, 1, riridx, ntraj]),
                                mode="full",
                                method="fft",
                            )
                            mixeventsig2 = scipy.signal.convolve(
                                eventsig,
                                np.squeeze(channel_rirs[:, 2, riridx, ntraj]),
                                mode="full",
                                method="fft",
                            )
                            mixeventsig3 = scipy.signal.convolve(
                                eventsig,
                        ...
                        if self._apply_event_gains:
                            ...
                            K = 1000
                            rand_energies_per_spec = utils.sample_from_quartiles(
                                K, self._class_gains[classidx]
                            )
                            intr_quart_energies_per_sec = rand_energies_per_spec[
                                K + np.arange(3 * (K + 1))
                            ]
                            ...
                            sample_active_time = (
                                sample_onoffsets[1] - sample_onoffsets[0]
                            )
                            target_energy = rand_energy_per_spec * sample_active_time
                            if self._audio_format == "mic":
                                event_omni_energy = np.sum(
                                    np.sum(mixeventsig, axis=1) ** 2
                                )
                        ...
                        if (
                            np.round(onoffset[0] * self._fs_mix) + lMixeventsig
                            <= self._t_mix * self._fs_mix
                        ):
                            mixsig[
                                int(np.round(onoffset[0] * self._fs_mix))
                                + np.arange(0, lMixeventsig, dtype=int),
                                :,
                            ] += mixeventsig
                        else:
                            ...
                            mixsig[
                                int(np.round(onoffset[0] * self._fs_mix))
                                + np.arange(0, lMixeventsig_trunc, dtype=int),
                                :,
                            ] += mixeventsig[
                                np.arange(0, lMixeventsig_trunc, dtype=int), :
                            ]
                    ...
                    mixsig = gnorm * mixsig
                    mixture_filename = "fold{}_room{}_mix{:03}.wav".format(
                        nfold + 1, nr + 1, nmix + 1
                    )
                    soundfile.write(
                        self._outpath + "/" + mixture_filename, mixsig, self._fs_mix
                    )
