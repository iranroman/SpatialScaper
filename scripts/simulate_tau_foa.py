...
for room_idx, room_name in enumerate(room_list):
    ...
    for i in range(n_traj):
        ...
        nroom_foa_rirs = len(os.listdir(os.path.join(path_to_room_foa_rirs, f'{room_name}_t{i}')))
        
        for j in range(nroom_foa_rirs):
            path_rirs, sr = soundfile.read(os.path.join(path_to_room_foa_rirs, f'{room_name}_t{i}', f'rir_{j}-foa.wav'))
