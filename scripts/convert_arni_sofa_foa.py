...
def create_single_sofa_file(aud_fmt, arni_db_dir, sofa_db_dir, room="ARNI"):
    ...
    path_to_room_foa_rirs = os.path.join(FOA_REAPER_PATH, 'arni')
    path_stack_traj = np.empty((0, 3))
    rir_stack_traj = np.empty((0, 4, 60000))

    nroom_foa_rirs = len(os.listdir(os.path.join(path_to_room_foa_rirs, f'arni_t0')))

    for j in range(nroom_foa_rirs):
        path_rirs, sr = soundfile.read(os.path.join(path_to_room_foa_rirs, f'arni_t0', f'rir_{j}-foa.wav'))
        path_rirs = path_rirs.T[np.newaxis,...]

        rir_stack_traj = np.concatenate((rir_stack_traj, path_rirs), axis=0)

    rirs = np.array(rir_stack_traj)
