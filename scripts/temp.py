...
def create_per_traj_sofa_file(aud_fmt, tau_db_dir, sofa_db_dir, db_name):
    ...
    for room_idx in range(9):
        ...
        for traj_idx in range(n_traj):
            # Load flattened (and flipped) rirs/paths from TAU-SRIR database
            rirs, source_pos, mic_pos, room = sofa_utils.load_flat_tau_srir(
                tau_db_dir, room_idx, aud_fmt=aud_fmt, traj=traj_idx
            )

            filepath = os.path.join(room_dir, f"{room}_t{traj_idx}.sofa")
            comment = (
                f"SOFA conversion of {room}, trajectory {traj_idx} from TAU-SRIR-DB"
            )

            print(
                f"Creating .sofa file for {aud_fmt}, Room: {room}, trajectory {traj_idx} (Progress: {room_idx + 1}/9, {traj_idx}/{n_traj})"
            )
