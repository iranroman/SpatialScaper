...
def generate_data(spatialize, audio_lengths, num_irs, ir_samples, sr, runs_per_test, desc=None, cache_file=None):
        ...
        for i, n_ir in enumerate(num_irs):
            for j, length in enumerate(audio_lengths):
                if n_ir in cache_ir and length in cache_al:
                    results[i, j] = cache_results[cache_ir.index(n_ir), cache_al.index(length)]
    ...
    try:
        for i, n_ir in enumerate(tqdm.tqdm(num_irs, desc=desc, leave=False)):
            for j, length in enumerate(tqdm.tqdm(audio_lengths, desc=f'N IRs: {n_ir}', leave=False)):
                if results[i, j] != 0:
                    tqdm.tqdm.write(f"[CACHE] {desc or ''} - {n_ir} IRs, {length} sec: {results[i, j]}")
                    continue
        ...
        if cache_file:
            np.savez(cache_file, results=results, audio_lengths=audio_lengths, num_irs=num_irs, ir_samples=ir_samples, sr=sr, runs_per_test=runs_per_test)
    return results
