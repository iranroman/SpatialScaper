        ...
        for nfold in range(self._nb_folds):
            ...
            for inr, nr in enumerate(rooms):  # range(nb_rooms):
                ...
                for nmix in range(nb_mixtures):
                    ...
                    target_omni_energy = (
                        np.sum(np.mean(target_sig, axis=1) ** 2)
                        if self._audio_format == "mic"
                        else np.sum(target_sig[:, 0] ** 2)
                    )

                    if self._scenarios[1]:
                        print(
                            "Loading interferer mixture {}/{} \n".format(
                                nmix + 1, nb_mixtures
                            )
                        )
                        ...
                        inter_omni_energy = (
                            np.sum(np.mean(interf_sig, axis=1) ** 2)
                            if self._audio_format == "mic"
                            else np.sum(interf_sig[:, 0] ** 2)
                        )
                        interf_norm = np.sqrt(
                            inter_omni_energy / (self._sir * inter_omni_energy)
                        )
                        ## ADD INTERFERENCE
                    ...
                    if self._scenarios[2]:
                        ...
                        if nmix < nSegs:
                            ...
                            ambient_sig = ambience[
                                rand_idx[0] * self._lMix + idx_range, :
                            ] / np.sqrt(2)
                            +ambience[
                                rand_idx[1] * self._lMix + idx_range, :
                            ] / np.sqrt(2)

                        ambi_energy = (
                            np.sum(np.mean(ambient_sig, axis=1) ** 2)
                            if self._audio_format == "mic"
                            else np.sum(ambient_sig[:, 0] ** 2)
                        )
                        ambi_norm = np.sqrt(
                            target_omni_energy * 10.0 ** (-snr / 10.0) / ambi_energy
                        )
