...
from .utils import (get_label_list, get_files_list, new_event_exceeds_max_overlap, 
                    count_leading_zeros_in_period, generate_trajectory, db2scale, 
                    traj_2_ir_idx, find_indices_of_change, IR_normalizer, spatialize, 
                    get_timegrid, get_labels, save_output, sort_matrix_by_columns)
from .sofa_utils import load_rir_pos, load_pos


# Sound event classes for DCASE Challenge
__DCASE_SOUND_EVENT_CLASSES__ = {
    'femaleSpeech': 0, 'maleSpeech': 1, 'clapping': 2, 'telephone': 3,
    'laughter': 4, 'domesticSounds': 5, 'footsteps': 6, 'doorCupboard': 7,
    'music': 8, 'musicInstrument': 9, 'waterTap': 10, 'bell': 11, 'knock': 12,
}
...
Event = namedtuple(
    ...
    'snr', 'role', 'pitch_shift', 'time_stretch','event_position'])
...
class Scaper:
    ...
    def add_background(self):
        ...
        self.bg_events.append(Event(label=None,
                                source_file=None,
                                source_time=None,
    ...
    def add_event(self,
        ...
        if event_position[1][0] == 'uniform' and moving:
            event_position = self.define_trajectory(event_position[1],int(event_duration/(1/self.label_rate)),shape)
    ...
    def define_event_onset_time(self,event_time, event_duration, other_events, max_overlap, increment):
        ...
        if new_event_exceeds_max_overlap(random_start_time, event_duration, other_events, max_overlap, increment):
            ...
            return self.define_event_onset_time(event_time, event_duration, other_events, max_overlap, increment)
        else:
    ...
    def generate_noise(self,audio):
        ...
        for event in self.bg_events:
            if not event.source_file:
                audio += db2scale(self.ref_db+event.snr)*np.random.normal(0,1,(int(event.event_duration*self.sr),self.nchans))
        return audio
    ...
    def generate(self, audiopath, labelpath):
        ...
        out_audio, labels = self.synthesize_events_and_labels(all_irs,all_ir_xyzs,out_audio)
