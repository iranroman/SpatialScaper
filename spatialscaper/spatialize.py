...
def stft_num_frames(y, hop_size=128):
    ...
    n_frames = 2 * int(np.ceil(y.shape[-1] / (2.0 * hop_size))) + 1  # original implementation
...
def _assert_shape_match(shape_a, shape_b, msg=None):
    assert all(b is None or a==b for a, b in zip(shape_a, shape_b)), f'{f"{msg}: " if msg else ""}{shape_a} != {shape_b}'
